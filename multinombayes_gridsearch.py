...
class Parameters :
    def __init__(self, lowerCaseFlag, removeStopWordsFlag, stemFlag, maxFeatures, ngramRange, tfidfFlags):
        ...
        self.alpha = -1 # setovano od inner search-a
...
def scoringFunction(estimator, x, y):
    ...
    print("The best estimator params", estimator.best_params_)
    predictions = estimator.best_estimator_.predict(x)
    ...
    classificationReport['alpha'] = estimator.best_params_['alpha']
...
def printAverageValuesOfClassificationReportList(outputStream, outputFileName, parameters, functionalMultipleSubCategories):
    ...
    for i in range(0, len(classificationReportList)):
        ...
        # alpha value is not same during outer cv (depends on inner cv) so set -1
        # classificationReportList[i]['alpha']
        parameters.alpha = -1
...
if __name__ == "__main__":
    ...
    for input_file, output_file_path, functionalMultipleSubCategories in [("svi sredjeni.txt", "outputNBGridSearch.csv", True), ("svi sredjeni functional.txt", "outputNBGridSearchFunctional.csv", False)]:
         with open(output_file_path, 'w') as output:
            ...
            for parameters in parametersList:
                ...
                param_grid = {'alpha': [2, 1, 0.5, 0.25, 0.1]}

                inner_cv = KFold(n_splits = 5, shuffle = True, random_state = 42)
                outer_cv = KFold(n_splits = 10, shuffle = True, random_state = 42)

                # Inner CV.
                objGS = GridSearchCV(naive_bayes.MultinomialNB(), param_grid, cv=inner_cv, refit = True, verbose = 1, n_jobs=8) #, 

                # Outer CV. Multinomial.fit() gets called in cross_validate.
                print(">>>>>>>>>>>>>>>>>>>>> c_v start. now =",  datetime.now())
                objCv = cross_validate(objGS, X=matrix, y=Corpus['Class'], scoring = scoringFunction, cv = outer_cv, return_train_score = False, verbose=1)
